...
void FullyConnectedNetworksBuilder::prepareAddNetworks(const QString& networkName, const QString& networkDescription, const bool* stop){
...
void FullyConnectedNetworksBuilder::addBasicNetwork(const QString& networkName, const QString& networkDescription){
	//Reset this class
	reset();
	...
	neuronList.append(neuronGroup.addNeuron(6, 3, 1));
	neuronList.append(neuronGroup.addNeuron(6, 4, 1));
	neuronList.append(neuronGroup.addNeuron(5, 5, 1));
	neuronList.append(neuronGroup.addNeuron(4, 6, 1));
	neuronList.append(neuronGroup.addNeuron(3, 6, 1));
	neuronList.append(neuronGroup.addNeuron(2, 5, 1));
	neuronList.append(neuronGroup.addNeuron(1, 4, 1));
	neuronList.append(neuronGroup.addNeuron(1, 3, 1));
	neuronList.append(neuronGroup.addNeuron(2, 2, 1));
...
void FullyConnectedNetworksBuilder::addConnections(){
	...
	for(unsigned int toNeurID=1; toNeurID<=12; ++toNeurID){
		...
		foreach(unsigned int fromNeurID, fromNeurIDList){
			...
			conListArray[toNeurID-1].append( connGrp.addConnection( new Connection(neuronMap[fromNeurID], neuronMap[toNeurID],  0,  0,  0) ) );
		}
	}
...
void FullyConnectedNetworksBuilder::addFiringPatterns(){
...
void FullyConnectedNetworksBuilder::addTraining(unsigned int percentInputs){
	//Number of connections to each neuron
	int numberOfConnections = 6;

	//Neuron will output 1 if the number o input neurons firing is greater than or equal to the threshold
...
QString FullyConnectedNetworksBuilder::getFiringNeuronStr(unsigned int percentNeurons){
...
QList<unsigned int> FullyConnectedNetworksBuilder::getRandomFromNeuronIDs(unsigned int toNeuronID, int numFromCons){
	...
	while (selectionMap.size() < numFromCons){
		//Fill map with indexes of selected neurons
		unsigned int tmpNeurID = rand() % 12 + 1;
...
void FullyConnectedNetworksBuilder::reset(){
	firingNeuronStr25 = "";
	firingNeuronStr50 = "";
	firingNeuronStr75 = "";
	firingNeuronStr100 = "";
