...
void StateBasedPhiAnalysisDao::deleteTimeSteps(unsigned int analysisID, unsigned int firstTimeStep, unsigned int lastTimeStep){
	executeQuery("DELETE FROM StateBasedPhiData WHERE AnalysisID=" + QString::number(analysisID) + " AND TimeStep >= " + QString::number(firstTimeStep) + " AND TimeStep <= " + QString::number(lastTimeStep));
}


/*! Returns the number of complexes within a particular range of time steps */
unsigned int StateBasedPhiAnalysisDao::getComplexCount(unsigned int analysisID, unsigned int firstTimeStep, unsigned int lastTimeStep){
