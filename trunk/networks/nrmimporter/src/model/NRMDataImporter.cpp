...
NRMDataImporter::NRMDataImporter(const DBInfo& networkDBInfo, const DBInfo& archiveDBInfo){
	...
	//Create Neuron and synapse types
	synapseType = Globals::getNetworkDao()->getSynapseType(WEIGHTLESS_SYNAPSE_TYPE_ID);
}
...
void NRMDataImporter::prepareAddArchives(NRMNetwork* nrmNetwork, Network* network, NRMDataSet* nrmDataset){
	...
	if(nrmDataset->size() % nrmNetwork->getNumberOfLayers() != 0){
		qDebug()<<"NRM dataset size: "<<nrmDataset->size()<<"; NRM network size: "<<nrmNetwork->getNumberOfLayers();
...
void NRMDataImporter::addConnections(){
	...
	for(int i=0; i<neuralList.size(); ++i){
		...
		for(int j=0; j<nrmConnGrpList.size(); ++j){
			...
			ConnectionGroup* newConGrp = new ConnectionGroup( ConnectionGroupInfo(0, "Untitled", nrmFromLayer->spikeStreamID, nrmToLayer->spikeStreamID, QHash<QString, double>(), synapseType) );
...
//			}
			...
			for(int patIndex = 0; patIndex < neuronConnectionList.size(); ++patIndex){
				...
				QList< QPair<unsigned, Connection> > tmpConPairList = networkDao.getConnections(tmpFromNeurID, neuronID);

				//Check that there is at least one connection
				if(tmpConPairList.isEmpty())
				...
				if(duplicateMap.contains(tmpFromNeurID)){
					//Double check size
					if(duplicateMap[tmpFromNeurID] >= tmpConPairList.size())
						throw SpikeStreamException("Duplicate map entry for neuron " + QString::number(tmpFromNeurID) + " is out of range.");

					networkDao.addWeightlessConnection(tmpConPairList[ duplicateMap[tmpFromNeurID] ].first, patIndex);
					++duplicateMap[tmpFromNeurID];//Increase the index that will be used next time the duplicate is encountered
				}
				else{
					networkDao.addWeightlessConnection(tmpConPairList[0].first, patIndex);
