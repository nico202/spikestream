...
void TestNRMConfigLoader::testLoadInputs(){
	QString fileName = workingDirectory + "multiple_input_test.cfg";
...
void TestNRMConfigLoader::testLoadNeuralLayers(){
	QString fileName = workingDirectory + "multiple_neural_test.cfg";
...
void TestNRMConfigLoader::testLoadConnectionParameters(){
	...
	//Load up the test configuration
	fileName = workingDirectory + "connection_parameters/randomdist_lincols_12conns.cfg";
	try{
		configLoader.loadConfig(fileName.toStdString().data());
	...
	//Load up the test configuration
	fileName = workingDirectory + "connection_parameters/iconic_4w5h_flipvertical_6conns.cfg";
	try{
		configLoader.loadConfig(fileName.toStdString().data());
	...
	//Load up the test configuration
	fileName = workingDirectory + "connection_parameters/global_spatial_9-10-11-12_7conns.cfg";
	try{
		configLoader.loadConfig(fileName.toStdString().data());
	...
	//Load up the test configuration
	fileName = workingDirectory + "connection_parameters/seg_segrev_rancols_11conns.cfg";
	try{
		configLoader.loadConfig(fileName.toStdString().data());
	...
	//Load up the test configuration
	fileName = workingDirectory + "connection_parameters/2connections.cfg";
	try{
		try{
			configLoader.loadConfig(fileName.toStdString().data());
		}
		catch(NRMException& ex){
			//Throws a memory related exception because of the 76832 connections
			QVERIFY(ex.getMessage().toUpper().contains("MEMORY"));
		}
